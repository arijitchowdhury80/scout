...
| ~~F2~~ | **CORRECTED 2026-07-27 — FALSE ALARM. Signup is NOT broken.** The 502 was Resend rejecting the fake `example.com` test address (Resend blocks it by design). Root-caused via Resend API: domain `scout.chowmes.com` is **verified**, not sandbox. Live prod evidence: real signups to gmail + `arijit.chowdhury@algolia.com` return `delivered`; duplicate → `account_exists`; reissue → `delivered` + key rotated. Signup, delivery, duplicate, status, and reissue all verified working on prod. | prod `hosted_signup_events` (5+ `delivered`); Resend domains API | The only real sub-issue was cosmetic: the raw SMTP error leaked to the UI — fixed in FX-5a (friendly message, real error logged server-side). |
