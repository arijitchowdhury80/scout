...
the hosted tenant that created the run. In this private-beta foundation,
ownership is enforced by the tenant-labeled output directory generated by the
hosted run endpoint. A production deployment should add explicit tenant columns
to run persistence before dashboard launch.
