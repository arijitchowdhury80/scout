...
| 3 | Hosted beta offer | Reject arbitrary `$22`/`$9`; fill unit economics before approving hosted pricing | Protects hosting/browser/LLM/security/support economics and avoids unlimited crawling liability | Keep website/payment copy metered; do not advertise unlimited hosted crawling |
...
| 6 | Stripe test-mode credentials | Provide when ready to close payment gate | Required to verify real Checkout Session, webhook, and hosted key delivery | Run `scripts/stripe_test_mode_smoke.py --create-checkout`, complete payment, deliver webhook, verify `/v1/hosted/me` |
...
3. Hosted beta: keep hosted usage metered; approve pricing only after the unit-economics model is filled.
